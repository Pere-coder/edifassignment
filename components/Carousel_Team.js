...
function Carousel_Team() {
  ...
  const item = members.map((item) => {
    return (
      <div className="item  w-auto md:w-full flex flex-col md:flex-row justify-center items-center " key={item.id} data-value={item.id}>
        <div className=" object-contain md:h-[450px] w-[250px] md:w-[1900px] rounded-xl overflow-hidden">
          <Image
            src={item.img}
            width={350}
            height={400}
  ...
  return (
    <div className=" min-h-screen w-full mt-5 md:mt-2  text-sblack flex flex-col items-center justify-center gap-4">
      <div>
